...
const steps: OnboardingStep[] = [
  ...
  },
  {
    ...
    icon: <FaWallet className="w-16 h-16" />
  },
  {
    id: 'play',
    title: 'Play & Learn',
...
export default function OnboardingFlow({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    ...
    >
      <motion.div
        className="bg-purple-900/20 backdrop-blur-xl rounded-2xl p-8 max-w-md w-full border border-purple-500/20"
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 0.1 }}
      >
        {/* Progress Bar */}
        <div className="h-2 bg-purple-900/30 rounded-full mb-8 overflow-hidden">
          <motion.div
